...
class MaintenanceRequest(models.Model):
    ...
    joborder_id = fields.Many2one("project.task", required=False,)
    requisition_employee_id = fields.Many2one(
        "hr.employee", string="Material Requisition Employee",
    )
    is_task = fields.Boolean(string="Create Task", default=False, copy=False,)
    is_requisition = fields.Boolean(
        string="Create Requisition", default=False, copy=False,
    )
    ...
    def create_task(self):
        for rec in self:
            ...
            rec.is_task = True
        res = self.env.ref(
            "website_job_workorder_request.action_website_job_workorder_request"
        )
        res = res.read()[0]
    ...
    def create_purchase_requisition(self):
        for rec in self:
            ...
            if not rec.requisition_employee_id.department_id:
                raise UserError(
                    _("Select Department on Material Requisition Employee.")
                )
            if rec.description:
