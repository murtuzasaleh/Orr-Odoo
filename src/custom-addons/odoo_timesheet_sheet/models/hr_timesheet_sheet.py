...
class HrTimesheetSheet(models.Model):
    ...
    def _default_date_from(self):
        ...
        if r == "month":
            ...
            return (datetime.today() + relativedelta(weekday=0, days=-6)).strftime(
                "%Y-%m-%d"
            )
        elif r == "year":
    ...
    def _default_date_to(self):
        ...
        if r == "month":
            return (
                datetime.today() + relativedelta(months=+1, day=1, days=-1)
            ).strftime("%Y-%m-%d")
        elif r == "week":
        ...
        for sheet in self:
            ...
            if new_user_id:
                ...
                if sheet.date_from >= sheet.date_to:  # 28/11/2019
                    raise ValidationError(
                        _("Date from can not be set higher then Date to.")
                    )  # 28/11/2019
    ...
    def action_timesheet_confirm(self):
        for sheet in self:
            if (
                ...
                sheet.message_subscribe(
                    [sheet.employee_id.parent_id.user_id.partner_id.id]
                )
