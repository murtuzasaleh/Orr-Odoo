...
class AccountAnalyticLine(models.Model):
    ...
    sheet_id = fields.Many2one(
        "hr_timesheet_sheet.sheet", compute="_compute_sheet", string="Sheet", store=True
    )
    ...
    def create(self, vals):
        ...
        if type(vals["date"]) == type("str"):
            vals_date = datetime.strptime(
                vals["date"], tools.DEFAULT_SERVER_DATE_FORMAT
            ).date()
        else:
            vals_date = datetime.strptime(
                vals["date"].strftime(tools.DEFAULT_SERVER_DATE_FORMAT),
                tools.DEFAULT_SERVER_DATE_FORMAT,
            ).date()
        # vals_date = datetime.strptime(vals['date'], tools.DEFAULT_SERVER_DATE_FORMAT).date()
        if vals.get("sheet_id"):
            hr_timesheet_obj = self.env["hr_timesheet_sheet.sheet"].browse(
                vals.get("sheet_id")
            )
            if (
                not vals_date >= hr_timesheet_obj.date_from
                or not vals_date <= hr_timesheet_obj.date_to
            ):
