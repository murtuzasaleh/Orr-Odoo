...
class MaterialPurchaseRequisition(models.Model):
    ...
    request_date = fields.Date(
        string="Requisition Date", default=fields.Date.today(), required=True,
    )
    department_id = fields.Many2one(
        "hr.department", string="Department", required=True, copy=True,
    )
    ...
    approve_manager_id = fields.Many2one(
        "hr.employee", string="Department Manager", readonly=True, copy=False,
    )
    reject_manager_id = fields.Many2one(
        "hr.employee", string="Department Manager Reject", readonly=True,
    )
    ...
    location_id = fields.Many2one(
        "stock.location", string="Source Location", copy=True,
    )
    ...
    managerapp_date = fields.Date(
        string="Department Approval Date", readonly=True, copy=False,
    )
    manareject_date = fields.Date(
        string="Department Manager Reject Date", readonly=True,
    )
    ...
    analytic_account_id = fields.Many2one(
        "account.analytic.account", string="Analytic Account", copy=True,
    )
    dest_location_id = fields.Many2one(
        "stock.location", string="Destination Location", required=False, copy=True,
    )
    delivery_picking_id = fields.Many2one(
        "stock.picking", string="Internal Picking", readonly=True, copy=False,
    )
    requisiton_responsible_id = fields.Many2one(
        "hr.employee", string="Requisition Responsible", copy=True,
    )
    ...
    purchase_order_ids = fields.One2many(
        "purchase.order", "custom_requisition_id", string="Purchase Ordes",
    )
    custom_picking_type_id = fields.Many2one(
        "stock.picking.type", string="Picking Type", copy=False,
    )
    ...
    def set_department(self):
        for rec in self:
            ...
            rec.dest_location_id = (
                rec.employee_id.dest_location_id.id
                or rec.employee_id.department_id.dest_location_id.id
            )
