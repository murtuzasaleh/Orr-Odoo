...
class ResPartner(models.Model):
    ...
    rfi_product_id = fields.Many2one("product.product", string="Product",)
    ...
    rfi_ticket_count = fields.Integer(compute="_compute_rfi_ticket_count", store=True,)
    ...
    def show_ticket(self):
        for rec in self:
            res = self.env.ref(
                "project_request_for_information.action_request_information"
            )
            res = res.read()[0]
