...
class RequestInformation(models.Model):
    ...
    def create(self, vals):
        ...
        if vals.get("name", "New") == "New":
            vals["name"] = (
                self.env["ir.sequence"].next_by_code("request.information") or "New"
            )
        ...
        if vals.get("team_id") and not vals.get("team_leader_id"):
            vals["team_leader_id"] = (
                self.env["request.information.team"]
                .browse(vals.get("team_id"))
                .leader_id.id
            )
        ...
        return super(
            RequestInformation, self.with_context(context, mail_create_nolog=True)
        ).create(vals)
    ...
    def set_to_close(self):
        stage_id = self.env["request.information.stage.config"].search(
            [("stage_type", "=", "closed")]
        )
        if self.is_close != True:
            ...
            template = self.env.ref(
                "project_request_for_information.email_template_request_information"
            )
    ...
    def _stage_find(self, team_id=False, domain=None, order="sequence"):
        ...
        return self.env["request.information.stage.config"].search(
    ...
    type_ticket_id = fields.Many2one(
        "request.information.type", string="Type", copy=False,
    )
    ...
    timesheet_line_ids = fields.One2many(
        "account.analytic.line", "rfi_request_information_id", string="Timesheets",
    )
    is_close = fields.Boolean(
        string="Is Closed ?", track_visibility="onchange", default=False, copy=False,
    )
    ...
    team_id = fields.Many2one(
        ...
        string="RFI Team",
        default=lambda self: self.env["request.information.team"]
        .sudo()
        ._get_default_team_id(user_id=self.env.uid),
    )
    ...
    task_id = fields.Many2one("project.task", string="Job Order", copy=False,)
    ...
    comment = fields.Text(string="Customer/Supplier Comment", readonly=True,)
    ...
    subject_type_id = fields.Many2one(
        "request.information.subject", string="Subject", copy=True,
    )
    allow_user_ids = fields.Many2many("res.users", string="Allow Users")
    rfi_request_answer = fields.Text(string="RFI Answer",)
    ...
    rfi_survey_ids = fields.One2many(
        "survey.survey", "rfi_request_id", string="Request Information Surveys"
    )

    custome_client_user_id = fields.Many2one(
        "res.users", string="RFI Created User", readonly=True, track_visibility="always"
    )
    ...
    def message_new(self, msg_dict, custom_values=None):
        ...
        return super(RequestInformation, self).message_new(
