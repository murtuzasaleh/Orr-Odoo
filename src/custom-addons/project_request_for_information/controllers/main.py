...
class RequestInformation(http.Controller):
    ...
    def request_information_submitted(self, **post):
        ...
        if Partner:
            ...
            values = {"request_information": request_information}
            ...
            if len(attachment_list) > 0:
                group_msg = (
                    "Customer has sent %s attachments to this Request Information. Name of attachments are: "
                    % (len(attachment_list))
                )
                ...
                request_information.sudo().message_post(
                    body=_(group_msg), message_type="comment"
                )

            return request.render(
                "project_request_for_information.thanks_mail_send", values
            )
        else:
            return request.render("project_request_for_information.support_invalid", {})

    @http.route(
        ["/request_information_email/feedback/<int:order_id>"],
        type="http",
        auth="public",
        website=True,
    )
    def feedback_email(self, order_id, **kw):
        ...
        return request.render(
            "project_request_for_information.request_information_feedback", values
        )

    @http.route(["/request/feedback/success"], type="http", auth="public", website=True)
    def start_rating(self, **kw):
        partner_id = kw["partner_id"]
        kw_request_information = kw["request_information_id"]
        request_information_id = request.env["request.information"].browse(
            int(kw_request_information)
        )
...
class CustomerPortal(CustomerPortal):
    ...
    @http.route(
        ["/my/request_informations", "/my/request_informations/page/<int:page>"],
        type="http",
        auth="user",
        website=True,
    )
    def portal_my_request(self, page=1, **kw):
        ...
        request_informations = support_obj.sudo().search(
        ...
        return request.render(
            "project_request_for_information.display_request_informations", values
        )

    @http.route(
        ['/my/request_information/<model("request.information"):request_information>'],
        type="http",
        auth="user",
        website=True,
    )
    def my_ticket(self, request_information=None, **kw):
        ...
        if len(attachment_list) > 0:
            ...
            request_informations_obj.sudo().message_post(
                body=_(group_msg),
            ...
            return http.request.render(
                "project_request_for_information.successful_ticket_send", {}
            )
        ...
        return request.render(
            "project_request_for_information.display_request_information",
