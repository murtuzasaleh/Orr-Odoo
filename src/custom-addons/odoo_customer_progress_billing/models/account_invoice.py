...
class AccountInvoice(models.Model):
    ...
    def _set_remaining_progress_billing(self):
        for rec in self:
            ...
            rec.remaining_progress_billing = (
                rec.total_progress_billing - rec.previously_invoice
            )
    ...
    project_id = fields.Many2one(
        "account.analytic.account", string="Project", copy=False,
    )
    ...
    invoice_to_date = fields.Float(
        string="Invoice To Date", compute="_set_invoiceto_date", copy=False, store=True,
    )
    ...
    total_due = fields.Float(
        string="Total Due Now", compute="_set_total_due", copy=False, store=True,
    )
