...
class Project(models.Model):
    ...
    def _compute_maintenance_request_count(self):
        ...
        for project in self:
            project.maintenance_request_count = maintenance_request.search_count(
                [("id", "in", maintenance_request_ids.ids)]
            )
    ...
    maintenance_request_ids = fields.One2many(
        "maintenance.request", "custom_project_id",
    )
...
class ProjectTask(models.Model):
    ...
    def _compute_maintenance_request_count(self):
        ...
        maintenance_request_ids = self.mapped("maintenance_request_ids")
        for task in self:
            task.maintenance_request_count = maintenance_request.search_count(
                [("id", "in", maintenance_request_ids.ids)]
            )
    ...
    maintenance_request_ids = fields.One2many(
        "maintenance.request", "custom_job_order_id",
    )
