...
class AccountAnalyticLine(models.Model):
    ...
    is_payroll_paid = fields.Boolean(string="Is Payroll Paid", readonly=True,)
    ...
    def _get_timesheet_cost_hook(self, timesheet):
        ...
        contract_ids = self.env["hr.payslip"].get_contract(
            timesheet.employee_id, start_date, end_date
        )
        if contract_ids and timesheet.work_type_id:
            contract = self.env["hr.contract"].browse(contract_ids[0])
            worktype_ids = contract.work_type_ids.filtered(
                lambda wo: wo.work_type_id.id == timesheet.work_type_id.id
            )
            if worktype_ids:
