...
class HrPayslip(models.Model):
        ...
        for rec in self:
            ...
            for vtype in work_type:
                total_hour = sum(
                    [
                        p.unit_amount
                        for p in rec.timesheet_ids
                        if p.work_type_id == vtype
                    ]
                )
                if total_hour != 0.0:
                    vals = {
    ...
    timesheet_ids = fields.Many2many(
        "account.analytic.line", string="Timesheet Line", readonly=True,
    )
    ...
    timesheet_line_count = fields.Integer(compute="_timesheet_line_count", store=True,)
    ...
    def action_payslip_done(self):
        ...
        for rec in self:
            for line in rec.timesheet_ids:
                line.with_context(skip_warning=True).write(
                    {"is_payroll_paid": True, "custom_payslip_id": rec.id}
                )
    ...
    def show_paid_timesheetline(self):
        self.ensure_one()
        res = self.env.ref(
            "construction_contracting_payroll.action_analytic_line_custom"
        )
        res = res.read()[0]
