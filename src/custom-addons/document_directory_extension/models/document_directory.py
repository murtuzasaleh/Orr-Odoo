...
class DocumentDirectory(models.Model):
    ...
    parent_id = fields.Many2one("document.directory", string="Parent Directory")
    ...
    children_ids = fields.One2many("document.directory", "parent_id", string="Childs",)
    attachment_ids = fields.One2many(
        "ir.attachment", "directory_id", string="Attachments",
    )
    code = fields.Char(string="Directory Code", required=True,)
    sequence_id = fields.Many2one(
        "ir.sequence", string="Entry Sequence", readonly=True,
    )
    group_ids = fields.Many2many("res.groups", string="Groups",)
    dir_type = fields.Selection(
        [("view", "View"), ("dir", "Directory")], string="Type", default="dir",
    )
    ...
    def _compute_attachment(self):
        for rec in self:
            child_doc_ids = (
                self.env["document.directory"]
                .sudo()
                .search([("parent_id", "=", rec.id)])
            )
            doc_ids = child_doc_ids + rec
            attachment = (
                self.env["ir.attachment"]
                .sudo()
                .search_count([("directory_id", "in", doc_ids.ids)])
            )
    ...
    def show_attachment(self):
        ...
        self.ensure_one()
        child_doc_ids = (
            self.env["document.directory"].sudo().search([("parent_id", "=", self.id)])
        )
    ...
    def create(self, vals):
        ...
        vals.update({"sequence_id": seq_obj.id})
