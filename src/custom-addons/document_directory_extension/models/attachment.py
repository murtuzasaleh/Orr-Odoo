...
class Attachment(models.Model):
    ...
    number = fields.Char("Number", copy=False, readonly=True,)
    ...
    def action_attachment_send(self):
        ...
        try:
            template_id = ir_model_data.get_object_reference(
                "document_directory_extension", "email_template_document_attachment"
            )[1]
        except ValueError:
            template_id = False
        try:
            ...
            if directory.group_ids:
                ...
                if not flag:
                    raise ValidationError(
                        _("Sorry you don't have access for this document.'")
                    )
            if directory:
                aname = attachment.name  # vals.get('name', False)
                # vals['number'] = self.env['ir.sequence'].next_by_code(directory.sequence_id.code)
                attachment.number = self.env["ir.sequence"].next_by_code(
                    directory.sequence_id.code
                )
                attachment.update({"directory_id": directory.id})
                # vals.update({
    ...
    def write(self, vals):
        ...
        if model and res_id:
            ...
            if directory:
                ...
                vals.update({"directory_id": directory.id})
            else:
                vals["number"] = self.env["ir.sequence"].next_by_code(
                    "document.directory.seq"
                )
