...
class RecordStartStopTimerLines(models.Model):
    ...
    start_time = fields.Datetime(string="Start Time", copy=False, readonly=True,)
    stop_time = fields.Datetime(string="Stop Time", copy=False, readonly=True,)
    parent_id = fields.Many2one("record.start.stop.timer", string="Parent",)
    duration = fields.Float(
        string="Duration(Hours)", compute="_compute_duration", store=True,
    )
    pause_user_id = fields.Many2one(
        "res.users", string="Paused By", copy=False, readonly=True,
    )
    restart_user_id = fields.Many2one(
        "res.users", string="Restarted By", copy=False, readonly=True,
    )
