    ...
    def record_work_time(self, **kw):
        ...
        if record_id:
            if record_type == "start":
                ...
                record.write(
                    {"last_pause_time": False, "start_stop_ids": [(0, 0, vals)]}
                )
            else:
                start_line = False
                ...
                if start_line:
                    ...
                    if record_type in ["pause", "stop"]:
                        ...
                        l2_start_line = pause_obj.search(
                            l2_domain, order="start_time desc", limit=1
                        )
                        if l2_start_line:
                            l2_start_line.write(
                                {
                        ...
                        if record_type == "stop":
                            ...
                            if task:
                                ...
                                total_duration = "{0:02.0f}:{1:02.0f}".format(
                                    *divmod(total_duration * 60, 60)
                                )

                                split_start_time = start_line.start_time.split(" ")[
                                    1
                                ].split(":")
                                start_time = (
                                    split_start_time[0] + ":" + split_start_time[1]
                                )
                                split_end_time = start_line.stop_timer.split(" ")[
                                    1
                                ].split(":")
                            ...
                            record.write({"last_pause_time": record_time})
