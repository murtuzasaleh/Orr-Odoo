...
class ContractingDrawing(models.Model):
    ...
    image = fields.Binary(string="Drawing Image",)
    job_card_drawing_id = fields.Many2one("project.task", string="Job Card")
...
class ContractingDrawingDocument(http.Controller):
    @http.route(
        ["/open/contract_drawing_document/<int:res_id>"],
        type="http",
        auth="public",
        website=True,
    )
    def open_drawing_document(self, res_id, **post):
        if res_id:
            ...
            return request.render(
                "job_drawing_image_contracting.contracting_drawing_document", values
            )
