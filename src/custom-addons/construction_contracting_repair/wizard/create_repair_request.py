...
class RepiarRequestWizard(models.TransientModel):
    ...
    def create_repair_request(self):
        ...
        if action_id:
            ...
            action["domain"] = "[('id','in', [" + ",".join(map(str, repair_ids)) + "])]"
            return action
...
class RepairProductWizard(models.TransientModel):
    ...
    repair_id = fields.Many2one("repair.request.wizard", string="Repair Product",)
    product_id = fields.Many2one("product.product", string="Product",)
    name = fields.Char(string="Repair Description",)
    repair_team_id = fields.Many2one("machine.support.team", string="Repair Team",)
    responsible_user_id = fields.Many2one("res.users", string="Repair Responsible",)
    nature_service_id = fields.Many2one("service.nature", string="Nature of Service",)
    repair_type_ids = fields.Many2many("repair.type", string="Repair Type",)
