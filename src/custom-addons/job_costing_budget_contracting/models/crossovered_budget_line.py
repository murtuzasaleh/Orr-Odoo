...
class CrossOveredBudgetLine(models.Model):
    ...
    @api.depends(
        "jobcost_line_id",
        "jobcost_line_id.purchase_order_line_ids",
        "date_from",
        "date_to",
    )
        ...
        for rec in self:
            for line in rec.jobcost_line_id.purchase_order_line_ids:
                if (
                    rec.date_from
                    and rec.date_to
                    and line.order_id.date_order.date() >= rec.date_from
                    and line.order_id.date_order.date() <= rec.date_to
    ...
    @api.depends(
        "jobcost_line_id",
        "jobcost_line_id.account_invoice_line_ids",
        "date_from",
        "date_to",
    )
            ...
            for line in rec.jobcost_line_id.account_invoice_line_ids:
                if line.invoice_id.date_invoice:
                    if (
                        rec.date_from
                        and rec.date_to
                        and line.invoice_id.date_invoice >= rec.date_from
                        and line.invoice_id.date_invoice <= rec.date_to
                        and line.invoice_id.state in ["open", "paid"]
                    ):
                        rec.actual_vendor_quantity += line.quantity

    @api.multi
    @api.depends(
        "jobcost_line_id", "jobcost_line_id.timesheet_line_ids", "date_from", "date_to"
    )
        ...
        for rec in self:
            for line in rec.jobcost_line_id.timesheet_line_ids:
                if (
                    rec.date_from
                    and rec.date_to
                    and line.date >= rec.date_from
                    and line.date <= rec.date_to
                ):
    ...
    jobtype_id = fields.Many2one("job.type", string="Job Type",)
    ...
    actual_cost_unit = fields.Float(
        string="Actual Cost unit", compute="_compute_actual_cost_unit", store=True,
    )
    ...
    costsheet_id = fields.Many2one("job.costing", string="Cost Sheet",)
    ...
    material_planned_qyt = fields.Float(string="Material Planned Qty",)
    overhead_planned_qyt = fields.Float(string="Overhead Planned Qty",)
    labour_hour = fields.Float(string="Labour Hours",)
