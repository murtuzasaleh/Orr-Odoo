...
class JobcostInvoice(models.TransientModel):
    ...
    invoice_date = fields.Date(
        "Invoice Date", default=fields.date.today(), required=True,
    )
    material_invoice = fields.Boolean("Include Material Lines",)
    labour_invoice = fields.Boolean("Include Labour Lines",)
    overhead_invoice = fields.Boolean("Include Overhead Lines",)
    ...
    def create_jobcost_invoice(self):
        ...
        for rec in self:
            ...
            material_line_ids = []
            if (
                rec.labour_invoice != True
                and rec.material_invoice != True
                and rec.overhead_invoice != True
            ):
            ...
            if rec.material_invoice:
                ...
                for material_id in material_ids:
                    ...
                    if job_costing.billable_method == "based_on_avbq":
                        quantity = (
                            material_id.actual_invoice_quantity
                            - material_id.invoice_qty
                        )
                    ...
                    if material_id.billable == "billable" and quantity > 0.0:
                        ...
                        if material_id.product_id.id:
                            account_id = (
                                material_id.product_id.property_account_income_id.id
                                or material_id.product_id.categ_id.property_account_income_categ_id.id
                            )
                        #                        if not account_id:
                ...
                for labour_id in labour_ids:
                    ...
                    if labour_id.billable == "billable" and qty > 0.0:
                        ...
                        if labour_id.product_id.id:
                            labour_account_id = (
                                labour_id.product_id.property_account_income_id.id
                                or labour_id.product_id.categ_id.property_account_income_categ_id.id
                            )
            ...
            if rec.overhead_invoice:
                ...
                for overhead_id in overhead_ids:
                    ...
                    if job_costing.billable_method == "based_on_avbq":
                        qty = (
                            overhead_id.actual_invoice_quantity
                            - overhead_id.invoice_qty
                        )
                    elif job_costing.billable_method == "based_on_apq":
                    ...
                    if overhead_id.billable == "billable" and qty > 0.0:
                        ...
                        if overhead_id.product_id.id:
                            overhead_account_id = (
                                overhead_id.product_id.property_account_income_id.id
                                or overhead_id.product_id.categ_id.property_account_income_categ_id.id
                            )
