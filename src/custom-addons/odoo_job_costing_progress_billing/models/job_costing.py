...
class JobCosting(models.Model):
    ...
    def _invocie_count(self):
        ...
        for cost_sheet in self:
            cost_sheet.invoice_count = invoice_obj.search_count(
                [("job_cost_id", "=", cost_sheet.id)]
            )

    invoice_ids = fields.One2many("account.invoice", "job_cost_id", store=True,)
    ...
    billable_method = fields.Selection(
        ...
        selection=[
            ("based_on_apq", "Based On Actual Purchase Qty"),
