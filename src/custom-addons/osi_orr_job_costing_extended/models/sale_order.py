...
class SaleOrder(models.Model):
    ...
    def action_view_job_cost_sheet(self):
        self.ensure_one()
        action = self.env.ref("odoo_job_costing_management.action_job_costing").read()[
            0
        ]
        estimate_job = self.env["sale.estimate.job"].search(
            [("quotation_id", "=", self.id)]
        )
        action["domain"] = [("id", "in", estimate_job and estimate_job.jobcost_id.ids)]
        return action
    ...
    def _create_task_for_non_service_product(self, project, non_service_task_list):
        values = {
            "name": "Materials",
    ...
    def action_confirm(self):
        ...
        for so in self:
            ...
            if project and estimate_job:
                ...
                so._create_task_for_non_service_product(project, non_service_task_list)
        return result
...
class SaleAdvancePaymentInv(models.TransientModel):
    ...
    def _create_invoice(self, order, so_line, amount):
        ...
        if len(sale_orders) == 1:
            estimate_job = self.env["sale.estimate.job"].search(
                [("quotation_id", "=", sale_orders.id)], limit=1
            )
