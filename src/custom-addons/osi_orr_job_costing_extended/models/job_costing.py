...
class JobCosting(models.Model):
    ...
    def _compute_total_material_sale(self):
        for rec in self:
            rec.total_material_sale = sum(
                [(p.product_qty * p.sale_price) for p in rec.job_cost_line_ids]
            )
    ...
    def _compute_total_overhead_sale(self):
        for rec in self:
            rec.total_overhead_sale = sum(
                [(p.product_qty * p.sale_price) for p in rec.job_overhead_line_ids]
            )
    ...
    def _compute_total_labor_sale(self):
        for rec in self:
            rec.total_labor_sale = sum(
                [(p.hours * p.sale_price) for p in rec.job_labour_line_ids]
            )

    @api.depends("total_material_sale", "total_labor_sale", "total_overhead_sale")
    def _compute_total_sale(self):
        for rec in self:
            rec.total_sale = (
                rec.total_material_sale + rec.total_labor_sale + rec.total_overhead_sale
            )

    @api.depends(
        "total_sale", "jobcost_total",
    )
    def _compute_total_margin(self):
        for rec in self:
            rec.total_margin = rec.total_sale - rec.jobcost_total
    ...
    total_labor_sale = fields.Float(
        string="Total Labour Sale", compute="_compute_total_labor_sale", store=True,
    )
    ...
    total_margin = fields.Float(
        string="Margin", compute="_compute_total_margin", store=True,
    )
    sale_order_count = fields.Integer(
        string="Sale Order Ids", compute="_compute_sale_order_count", store=True,
    )
    ...
    def action_view_sale_orders(self):
        cost_estimate_ids = self.env["sale.estimate.job"].search(
            [("jobcost_id", "=", self.id)]
        )
        sale_order_ids = [
            estimate_id.quotation_id.id
            for estimate_id in cost_estimate_ids
            if estimate_id.quotation_id
        ]
...
class JobCostLine(models.Model):
    ...
    product_id = fields.Many2one(
        "product.product", string="Product", copy=True, required=True,
    )
    description = fields.Char(string="Description", copy=True,)
    reference = fields.Char(string="Reference", copy=True,)
    date = fields.Date(string="Date", required=True, copy=True,)
    product_qty = fields.Float(string="Planned Qty", copy=True,)
    cost_price = fields.Float(string="Cost / Unit", copy=True,)
    sale_price = fields.Float(string="Sale Price / Unit", copy=True,)
