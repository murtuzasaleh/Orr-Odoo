...
class GoogleDrawingLink(models.Model):
    ...
    url = fields.Char(string="Google Drawing Url", required=True)
...
class GoogleDrawingDocument(http.Controller):
    @http.route(
        ["/open/drawing_document/<int:res_id>"],
        type="http",
        auth="public",
        website=True,
    )
    def open_drawing_document(self, res_id, **post):
        if res_id:
            ...
            return request.render(
                "job_drawing_construction_contracting.google_drawing_document", values
            )
