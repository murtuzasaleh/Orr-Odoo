...
class website_account(website_account):
    def _prepare_portal_layout_values(self):
        ...
        joborder_count = (
            ...
            .sudo()
            .search_count([("job_partner_id", "=", partner.id)])
        )
        values.update(
    ...
    @http.route(
        ["/job_order/website_joborder_submitted"],
        type="http",
        auth="public",
        methods=["POST"],
        website=True,
    ...
    def joborder_submitted(self, **post):
        ...
        project_id = (
            ...
            .sudo()
            .search([("custome_code", "=", post.get("project_code"))], limit=1)
        )
        if project_id:
            vale.update(
        ...
        issue_template = http.request.env.ref(
            "website_job_workorder_request.email_template_job_order_send_view"
        )
        issue_template.sudo().with_context(local_context).send_mail(
            request.uid, force_send=False
        )
        ...
        values = {"order": workorder_id}
        return request.render(
            "website_job_workorder_request.website_thanks_mail_send", values
        )

    @http.route(
        ["/my/joborders", "/my/joborders/page/<int:page>"],
        type="http",
        auth="user",
        website=True,
    )
    def my_joborders(
        self, page=1, date_begin=None, date_end=None, joborder=None, sortby=None, **kw
    ):
        ...
        job_order_ids = (
            request.env["project.task"]
            .sudo()
            .search(domain, limit=self._items_per_page, offset=pager["offset"])
        )
        values.update(
            {
                "date": date_begin,
        ...
        return request.render(
            "website_job_workorder_request.my_portal_job_order", values
        )
    ...
    def joborder_print(self, joborder, access_token=None, **kw):
        ...
        pdf, _ = (
            request.env.ref("website_job_workorder_request.joborder_report")
            .sudo()
            .render_qweb_pdf([joborder])
        )
        pdfhttpheaders = [
