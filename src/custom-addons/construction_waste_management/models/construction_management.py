...
class ConstructionWasteManagement(models.Model):
    ...
    type = fields.Selection(
        [("reused", "Reuse Material"), ("scrap", "Scrap Material")],
        string="Waste Method",
        required=True,
        copy=False,
    )
    product_id = fields.Many2one(
        "product.product", string="Product", required=True, copy=False,
    )
    qty = fields.Integer(string="Quantity")
    uom_id = fields.Many2one("uom.uom", string="UOM", copy=False,)  # product.uom
    ...
    task_id = fields.Many2one(
        "project.task", string="Job Order", copy=False, required=True,
    )
    stock_scrap_id = fields.Many2one(
        "stock.scrap", string="Stock Scrap", copy=False, readonly=True,
    )
    is_waste_created = fields.Boolean(string="Is Waste Created ?", copy=False,)
    ...
    user_id = fields.Many2one(
        "res.users", default=lambda self: self.env.user.id, string="User",
    )
