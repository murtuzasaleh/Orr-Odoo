...
class SaleOrder(models.Model):
    ...
    def _action_confirm(self):
        ...
        for order in self:
            agreement_id = self.env["agreement"].search([("sale_id", "=", order.id)])
            if agreement_id:
            ...
            if project:
                for recurring_id in self.fsm_recurring_ids:
                    ...
                    for line_id in self.agreement_id.serviceprofile_ids:
                        if (
                            line_id.product_id
                            == recurring_id.sale_line_id.product_id.product_tmpl_id
                        ):
            ...
            for recurring_id in self.fsm_recurring_ids:
                for order_id in recurring_id.fsm_order_ids:
                    ...
                    if recurring_id.serviceprofile_id:
                        order_id.write(
                            {"serviceprofile_id": recurring_id.serviceprofile_id.id}
                        )
                    if recurring_id.fsm_equipment_id:
                        order_id.write(
                            {"equipment_id": recurring_id.fsm_equipment_id.id}
                        )
        ...
        if self.product_id.fsm_recurring_template_id.fsm_order_template_id:
            if self.product_id.fsm_recurring_template_id.fsm_order_template_id.team_id:
                res[
                    ...
                    self.product_id.fsm_recurring_template_id.fsm_order_template_id.team_id.id
                )
        return res
