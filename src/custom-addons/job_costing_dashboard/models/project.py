...
class Project(models.Model):
    ...
    def action_open_job_orders(self):
        action = self.env.ref("odoo_job_costing_management.action_view_job_orders")
        action_read = action.read([])[0]
        action_read["domain"] = [("project_id", "=", self.id)]
        action_read["context"] = {"default_project_id": self.id}
        return action_read

    @api.multi
    def action_open_cost_sheet(self):
        action = self.env.ref("odoo_job_costing_management.action_job_costing")
        action_read = action.read([])[0]
        action_read["domain"] = [("project_id", "=", self.id)]
        action_read["context"] = {"default_project_id": self.id}
        return action_read
    ...
    def action_open_sheet_materials(self):
        action = self.env.ref("job_costing_dashboard.action_material_cost_sheet_line")
        action_read = action.read([])[0]
        action_read["domain"] = [
    ...
    def action_open_sheet_labours(self):
        action = self.env.ref("job_costing_dashboard.action_labour_cost_sheet_line")
        action_read = action.read([])[0]
        action_read["domain"] = [
            ("direct_id.project_id", "=", self.id),
            ("job_type", "=", "labour"),
    ...
    def action_open_sheet_overheads(self):
        action = self.env.ref("job_costing_dashboard.action_overhead_cost_sheet_line")
        action_read = action.read([])[0]
        action_read["domain"] = [
            ("direct_id.project_id", "=", self.id),
            ("job_type", "=", "overhead"),
    ...
    def action_open_job_orders_po(self):
        ...
        line_ids = purchase_order_lines_obj.search(
