...
class ProjectTaskRiskLine(models.Model):
    ...
    @api.depends(
        "task_id", "risk_id", "task_id.name", "risk_id.name",
    )
    ...
    category_id = fields.Many2one("project.risk.category", "Category", required=True,)
    risk_id = fields.Many2one("project.risk", "Risk", required=True,)
    type_id = fields.Many2one("project.risk.type", "Risk Type", required=True,)
    probality = fields.Float(string="Probablity(%)",)
    response_id = fields.Many2one(
        "project.risk.response", "Risk Response", required=True,
    )
    description = fields.Char(string="Description")
    task_project_id = fields.Many2one(
        "project.project", "Project", related="task_id.project_id",
    )
