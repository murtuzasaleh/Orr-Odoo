...
class RiskTask(models.TransientModel):
    ...
    risk_task_image = fields.Binary("Incident Photo",)
    description = fields.Text(string="Description", required=True)
    name = fields.Char(string="Risk Incident", required=True)
    user_id = fields.Many2one(
        "res.users", string="Assigned to", default=lambda self: self.env.uid,
    )
    project_id = fields.Many2one(
        "project.project", string="Project", default=_default_project, required=True
    )
