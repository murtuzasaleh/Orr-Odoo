...
class VisitVisit(models.Model):
    ...
    contract_id = fields.Many2one(
        "contract.contract", related="contract_line_id.contract_id", string="Contract"
    )
    so_due_date = fields.Date(string="SO Due Date")
    recurrence_id = fields.Many2one("recurrence.recurrence", string="Recurrence")
    visit_type = fields.Selection(
        [("full", "Full"), ("medium", "Medium"), ("small", "Small")],
        string="Visit Type",
    )
    ...
    fsm_order_id = fields.Many2one("fsm.order", string="FSO")
    fso_status = fields.Char(related="fsm_order_id.stage_name", string="Status")
    ...
    def _recurring_create_sale(self):
        ...
        sales_values = []
        if not sale_rec:
            sale_values = self.contract_id._prepare_sale(self.so_due_date)
            sale_values.update(
                {
                    "contract_id": self.contract_id.id,
            ...
            if contract_line_values:
                sale_values["order_line"].append((0, 0, contract_line_values))
    ...
    def cron_contract_recurring_create_sale(self):
        ...
        for rec in visit_rec:
            ...
            if (
                rec.contract_id.advance_number == diff_time
                and rec.contract_id.state == "active"
            ):
                rec._recurring_create_sale()
            elif so_due_date == today and rec.contract_id.state == "active":
                rec._recurring_create_sale()
        return True
