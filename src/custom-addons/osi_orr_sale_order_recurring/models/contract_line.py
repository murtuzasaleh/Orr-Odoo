...
class ContractLine(models.Model):
    ...
    sale_rep_id = fields.Many2one("res.users", string="Sales Rep")
    is_subcontracted = fields.Boolean(string="Is Subcontracted")
    tm_flat_rate = fields.Float(string="T&M or Flat rate")
    frequency_id = fields.Many2one("sale.order.frequency", string="Frequency")
    so_frequency_line_ids = fields.One2many(
        string="Visit Setup",
        comodel_name="sale.order.frequency.line",
        inverse_name="contract_line_id",
    )
    no_future_visits = fields.Integer(string="No. of Future Visits", default=13,)
    visit_ids = fields.One2many(
        string="Visits", comodel_name="visit.visit", inverse_name="contract_line_id",
    )
    ...
    @api.multi
    def _recurrence_check(self, factor, visits_list, so_due_date):
        final_due_date = False
        for rec in self:
            exclusion_check_list = [visit.get("so_due_date") for visit in visits_list]
            while so_due_date in exclusion_check_list:
                final_due_date = so_due_date + relativedelta(months=factor)
                so_due_date = final_due_date
    ...
    def generate_visits(self):
        for rec in self:
            ...
            for freq_line in rec.so_frequency_line_ids.sorted(
                key=lambda f_line: f_line.recurrence_id.factor, reverse=True
            ):
                if not freq_line.recurrence_id.factor:
                    raise ValidationError(
                        _(
                            "%s Recurrence Factor is not Set."
                            % (freq_line.recurrence_id.name)
                        )
                    )
                ...
                new_due_date = False
                for no in range(0, floor_round_value):
                    visit_count += 1
                    if visit_count <= rec.no_future_visits:
                        if not no:
                            so_due_date = freq_line.first_service
                        else:
                            so_due_date = visits_list[-1].get(
                                "so_due_date"
                            ) + relativedelta(months=freq_line.recurrence_id.factor)
                        exclusion_check_list = [
                            visit
                            for visit in visits_list
                            if visit.get("so_due_date") == so_due_date
                            and not new_due_date
                        ]
                        if exclusion_check_list:
                            new_due_date = rec._recurrence_check(
                                freq_line.recurrence_id.factor, visits_list, so_due_date
                            )
                            visit_count -= 1
                        else:
                            if new_due_date:
