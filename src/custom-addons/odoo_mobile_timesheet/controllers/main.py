...
class CustomerPortal(CustomerPortal):
    ...
    def portal_my_timesheet(self, page=1, sortby=None, **kw):
        if not (
            ...
            and request.env.user.has_group("hr_timesheet.group_hr_timesheet_user")
        ):
        ...
        # count for pager
        timesheets_count = http.request.env["account.analytic.line"].search_count(
            domain
        )
        # pager
        ...
        timesheets = timesheets_obj.search(
    ...
    def portal_add_timesheet(
        self, page=1, date_begin=None, date_end=None, project=False, task=False, **kw
    ):
        if not (
            request.env.user.has_group("base.group_user")
            and request.env.user.has_group("hr_timesheet.group_hr_timesheet_user")
    ...
    def create_new_timesheet(self, **kwargs):
        if (
            ...
            return request.render("odoo_mobile_timesheet.not_allowed")
        valse = {"user_id": request.env.user.id}
        ...
        if kwargs.get("start_time"):
            start = datetime.strptime(
                str(kwargs.get("start_time")), "%H:%M"
            ) - datetime.strptime(str("0:0"), "%H:%M")
            start_time = start.total_seconds() / 3600.00
            valse.update({"start_time": start_time})
        if kwargs.get("end_time"):
            end = datetime.strptime(
                str(kwargs.get("end_time")), "%H:%M"
            ) - datetime.strptime(str("0:0"), "%H:%M")
        ...
        if kwargs.get("quantity"):
            ...
            try:
                date_tt = datetime.strptime(quantity_str, "%H:%M") - datetime.strptime(
                    str("0:0"), "%H:%M"
                )
            except:
        ...
        return request.redirect(
            "/odoo_timesheet_portal_user_employee/select_timesheet?start_date="
            + str(kwargs.get("start_date"))
        )

    @http.route(
        ["/my/timesheet/<int:timesheet>"], type="http", auth="user", website=True
    )
    def edit_timesheet(self, timesheet=None, **kw):
        ...
        line_date = datetime.strptime(str(analytic_line.date), "%Y-%m-%d").strftime(
            "%Y-%m-%d"
        )
        work_type_ids = request.env["timesheet.work.type"].search([])
        values = {
    ...
    def update_timesheet(self, **kwargs):
        ...
        if kwargs.get("line_id"):
            line_id = request.env["account.analytic.line"].browse(
                int(kwargs.get("line_id"))
            )
    ...
    @http.route(
        ["/my/timesheet/delete/<int:timesheet>"], type="http", auth="user", website=True
    )
    ...
    @http.route(
        ["/odoo_timesheet_portal_user_employee/select_timesheet"],
        auth="user",
        website=True,
    )
    def select_timesheet(self, page=1, sortby=None, **kw):
        ...
        if kw.get("start_date"):
            ...
            if kw.get("search"):
                domain += [
                    "|",
                    ("date", "ilike", kw.get("search")),
                    ("name", "ilike", kw.get("search")),
                ]
            order = sortings.get(sortby, sortings["date"])["order"]
