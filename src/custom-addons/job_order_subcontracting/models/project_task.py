...
class ProjectTask(models.Model):
    ...
    purchaseorder_line_ids = fields.One2many(
        "order.line", "sub_contractor_id", string="Material Plan",
    )
    subcontractor_id = fields.Many2one("project.task", string="Subcontractor Joborder",)
    custom_partner_id = fields.Many2one(
        "res.partner", string="Subcontractor", readonly=True,
    )
    is_subcontractor_joborder = fields.Boolean(
        string="Subcontractor Job order", copy=False, default=False, readonly=True,
    )
