...
class ConstructionChngeOrderLine(models.Model):
    ...
    uom_id = fields.Many2one("uom.uom", string="Uom",)  # product.uom
    description = fields.Text(string="Description", required="True",)
    sale_price = fields.Float(
        string="Sale Price", digits=dp.get_precision("Product Price"), default=0.0
    )
    change_order_id = fields.Many2one(
        "construction.change.order", string="Construction Change",
    )
    currency_id = fields.Many2one(
        "res.currency", string="Currency", related="change_order_id.currency_id",
    )
    subtotal = fields.Float(string="Subtotal", compute="_compute_amount", store=True,)
    ...
    def _get_display_price(self, product):
        ...
        price, rule_id = self.change_order_id.pricelist_id.get_product_price_rule(
            self.product_id, self.quantity or 1.0, self.change_order_id.partner_id
        )
        pricelist_item = self.env["product.pricelist.item"].browse(rule_id)
        if (
            pricelist_item.base == "pricelist"
            and pricelist_item.base_pricelist_id.discount_policy == "with_discount"
        ):
            ...
            return from_currency.compute(
                product.lst_price, self.change_order_id.pricelist_id.currency_id
            )
    ...
    def _onchange_product(self):
        ...
        for rec in self:
            if rec.product_id:
                ...
                if (
                    self.change_order_id.pricelist_id
                    and self.change_order_id.partner_id
                ):
                    a = self._get_display_price(rec.product_id)
                    vals["sale_price"] = self.env[
                        "account.tax"
