...
class ConstructionChangeOrder(models.Model):
    ...
    def _compute_newamount(self):
        for rec in self:
            rec.total_contract_amount_all_change = (
                rec.original_contract_amount + rec.amount_total
            )
    ...
    date = fields.Datetime(
        string="Create Date", default=fields.Datetime.now(), required=True,
    )
    ...
    analytic_account_id = fields.Many2one(
        "account.analytic.account", string="Contract/Analytic Account", required=True,
    )
    reason_note = fields.Text(string="Reason for Change",)
    order_line_ids = fields.One2many(
        "construction.change.order.line", "change_order_id", string="Change order line",
    )
    ...
    total = fields.Float(strong="Total", compute="_compute_total", store=True,)
    note = fields.Text(string="Note",)
    term = fields.Text(string="Terms and Conditions",)
    approve_by = fields.Many2one(
        "res.users", string="Approved By", readonly=True, copy=False,
    )
    approve_date = fields.Date(string="Approved Date", readonly=True, copy=False,)
    customer_approve = fields.Many2one(
        "res.users", string="Customer Approved", readonly=True, copy=False,
    )
    customer_approve_date = fields.Date(
        string="Customer Approved Date", readonly=True, copy=False,
    )
    ...
    done_date = fields.Date(string="Closed Date", readonly=True, copy=False,)
    guarantor_1 = fields.Many2one("res.partner", string="Guarantor One")
    guarantor_2 = fields.Many2one("res.partner", string="Guarantor Two")
    original_contract_amount = fields.Float(string="Original Contract Amount")
    ...
    original_job_completion_date = fields.Date(
        string="Original Job Completion Date", copy=False, default=fields.date.today(),
    )
    ...
    is_saleorder = fields.Boolean(string="Is Sale order", default=False)
