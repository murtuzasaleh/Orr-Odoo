...
class IrAttachment(models.Model):
    ...
    prev_attachment_id = fields.Many2one(
        "ir.attachment", string="Previous Version Document", readonly=False, copy=False,
    )
    new_attachment_id = fields.Many2one(
        "ir.attachment", string="New Version Document", readonly=True, copy=False,
    )
    custom_version = fields.Float(
        string="Document Version", copy=False, default=0.0, digits=(4, 1),
    )
    custom_display_name = fields.Char(string="Display Name", compute="_custom_version")
    ...
    def _custom_version(self):
        for record in self:
            for rec in record:
                rec.custom_display_name = (
                    (rec.name or "") + "(" + str(rec.custom_version or "") + ")"
                )
    ...
    def search_read(self, domain=None, fields=None, offset=0, limit=None, order=None):
        ...
        config = (
            self.env["reduce.model.versioning"]
            .sudo()
            ...
            if not self._context["params"]["action"] == action.id:
                for attach in result:
                    if attach.get("id") and attach.get("name"):
                        ...
                        attach["name"] = (
                            attach["name"] + " (" + str(custom_version) + ")"
                        )
        return result
    ...
    def create(self, vals):
        ...
        # attachment_id = super(IrAttachment, self).create(vals)
        if ir_attachment:
            ir_attachment.write({"new_attachment_id": attachment.id})
        return attachment
