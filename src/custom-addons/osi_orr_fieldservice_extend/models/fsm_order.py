...
class FSMOrder(models.Model):
    ...
    def build_bill(self):
        ...
        message = _(
            "This Purchase Order has been created from: <a href=# data-oe-model=account.invoice data-oe-id=%d>%s</a><br"
        ) % (self.id, self.name)
        ...
        for line in self.sale_order_line_ids:
            ...
            invoice_line.write(
                {
                    "price_unit": po_id.order_line[index].price_unit,
                    "account_id": invoice_line.product_id.property_account_expense_id.id,
                }
            )
            index += 1
        # Raise warning if no invoice lines
    ...
    bill_count = fields.Integer(
        string="Vendor Bill Count", compute="_compute_vendor_bill_count", readonly=True
    )
    ...
    def _compute_invoice_buttons(self):
        if self.bill_count == 0:
            ...
            for inv in self.invoice_ids:
                if inv.type == "in_invoice" and inv.state in [
                    "draft",
                    "open",
                    "in_payment",
                    "paid",
        ...
        for rec in self:
            if duration and rec.group_id:
                fsm_order_rec = self.search(
                    [("group_id", "=", rec.group_id.id), ("id", "!=", rec.id)]
                )
                for fsm_rec in fsm_order_rec:
                    new_date = fsm_rec.scheduled_date_start + relativedelta(
                        hours=duration
                    )
                    ...
                    self._cr.execute(
                        ...
                        WHERE id = %s""",
                        (new_date, fsm_end_date, fsm_rec.id,),
                    )
            if vals.get("person_id", False) and rec.stage_id.name in [
                "New",
                "Scheduled",
            ]:
