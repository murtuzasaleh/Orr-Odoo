...
class FSMLocation(models.Model):
    ...
    fso_count = fields.Integer(string="FSM Orders", compute="_compute_fso_count")

    @api.multi
    def _compute_fso_count(self):
        for loc in self:
            loc.fso_count = self.env["fsm.order"].search_count(
                [("location_id", "=", loc.id)]
            )
    ...
    so_count = fields.Integer(string="Sales Orders", compute="_compute_so_count")

    @api.multi
    def _compute_so_count(self):
        for loc in self:
            loc.so_count = self.env["sale.order"].search_count(
    ...
    rfso_count = fields.Integer(
        string="Recurring FSM Orders", compute="_compute_rfso_count"
    )

    @api.multi
    def _compute_rfso_count(self):
        for loc in self:
            loc.rfso_count = self.env["fsm.recurring"].search_count(
                [("location_id", "=", loc.id)]
            )
    ...
    agreement_count = fields.Integer(
        string="Agreements", compute="_compute_agreement_count"
    )

    @api.multi
    def _compute_agreement_count(self):
        for loc in self:
            loc.agreement_count = self.env["agreement"].search_count(
                [("fsm_location_id", "=", loc.id)]
            )
