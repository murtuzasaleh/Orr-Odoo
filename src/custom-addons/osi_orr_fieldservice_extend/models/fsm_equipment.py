...
class FSMEquipment(models.Model):
    ...
    def action_view_fsm_order(self):
        ...
        fsm_order_rec = self.env["fsm.order"].search(
            [("location_id", "=", self.location_id.id), ("equipment_id", "=", self.id)]
        )
        action["domain"] = [("id", "in", fsm_order_rec and fsm_order_rec.ids)]
        return action
    ...
    def action_view_fsm_recurring(self):
        ...
        action["domain"] = [("id", "in", fsm_recurring_rec and fsm_recurring_rec.ids)]
        return action
