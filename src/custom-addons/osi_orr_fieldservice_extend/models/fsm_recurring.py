...
class FSMRecurringOrder(models.Model):
    ...
    group_id = fields.Many2one("fsm.recurring.group", string="Recurring Group ID")

    def _prepare_order_values(self, date=None):
        values = super(FSMRecurringOrder, self)._prepare_order_values(date)
        if values.get("fsm_recurring_id") and values.get("scheduled_date_start"):
            ...
            if recurring_rec.group_id:
                ...
                group_name = recurring_rec.group_id.name + "-" + scheduled_date_start
            ...
            if vals.get("group_id"):
                ...
                for order_rec in rec.fsm_order_ids:
                    if order_rec.group_id:
                        group_last_index = order_rec.group_id.name.split("-")[-1]
