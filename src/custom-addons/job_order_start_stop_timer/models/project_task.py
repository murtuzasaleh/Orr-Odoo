...
class ProjectTask(models.Model):
    ...
    @api.depends(
        "start_stop_ids", "start_stop_ids.total_duration",
    )
    ...
    @api.depends(
        "start_stop_ids", "start_stop_ids.start_time", "start_stop_ids.stop_timer"
    )
    ...
    start_stop_ids = fields.One2many(
        "record.start.stop.timer", "joborder_id", string="Start Stop", readonly=False
    )
    last_pause_time = fields.Datetime(
        string="Last Pause Time", copy=False, readonly=True,
    )
    latest_start_time = fields.Datetime(
        "Latest Start Datetime", compute="_compute_latest_time"
    )
    latest_stop_time = fields.Datetime(
        "Latest Stop Datetime", compute="_compute_latest_time"
    )
    total_duration = fields.Float(
        string="Total Duration", compute="_compute_total_duration",
    )
