...
class JobCostSheetImport(models.TransientModel):
    ...
    def import_job_costsheet_file(self):
        ...
        analytic_id = analytic_account_obj.search(
            [("name", "=", analytic_account)], limit=1
        )
        task_id = task_obj.search([("name", "=", task)], limit=1)

        if not partner_id:
            raise ValidationError(
                _("Partner not found for %s" % (sheet.cell(6, 2).value))
            )
                ...
                if material_row != False:
                    while sheet.cell(row, 0).value != "Labours":
                        ...
                        if not date:
                            raise ValidationError(
                                _(
                                    "Date not found for %s"
                                    % (sheet.cell(line_row, 0).value)
                                )
                            )

                        job_type_id = self.env["job.type"].search(
                        ...
                        if job_costsheet_id:
                            ...
                            jcs_line_values = jobcost_line_new._convert_to_write(
                                {
                                    name: jobcost_line_new[name]
                                    for name in jobcost_line_new._cache
                                }
                            )
                            ...
                            jobcostsheet_line = costsheet_line_obj.create(
                                jcs_line_values
                            )
                            row = row + 1
