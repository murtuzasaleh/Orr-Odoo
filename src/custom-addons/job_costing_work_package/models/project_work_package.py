...
class ProjectWorkPackage(models.Model):
    ...
    date = fields.Date(
        string="Date", default=fields.date.today(), copy=True, required=True,
    )
    ...
    project_id = fields.Many2one(
        "project.project", string="Project", required=True, copy=True,
    )
    task_ids = fields.Many2many(
        "project.task", string="Work Package Lines", copy=False,
    )
    number = fields.Char(string="Number", readonly=True, copy=False)
    partner_id = fields.Many2one(
        "res.partner", string="Customer", copy=True, required=True,
    )
    ...
    def unlink(self):
        for record in self:
            if record.state != "draft":
                raise UserError(
                    "You can not delete record which is not in draft state."
                )
    ...
    def action_send(self, vals):
        ...
        try:
            template_id = self.env.ref(
                "job_costing_work_package.email_work_packages_templates"
            ).id
        except ValueError:
            template_id = False
        try:
