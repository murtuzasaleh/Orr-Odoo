...
class MaterialPlanning(models.Model):
    ...
    product_uom = fields.Many2one("uom.uom", "Unit of Measure")  # product.uom
...
class ConsumedMaterial(models.Model):
    ...
    consumed_task_material_id = fields.Many2one(
        "project.task", string="Consumed Material Plan Task"
    )
...
class ProjectTask(models.Model):
    ...
    picking_ids = fields.One2many(
        "material.purchase.requisition", "task_id", string="Stock Pickings"
    )
    ...
    material_plan_ids = fields.One2many(
        "material.plan", "material_task_id", string="Material Plannings"
    )
    consumed_material_ids = fields.One2many(
        "consumed.material", "consumed_task_material_id", string="Consumed Materials"
    )
    stock_moves_count = fields.Integer(
        compute="total_stock_moves_count", string="# of Stock Moves", store=True,
    )
    parent_task_id = fields.Many2one(
        "project.task", string="Parent Task", readonly=True
    )
    child_task_ids = fields.One2many(
        "project.task", "parent_task_id", string="Child Tasks"
    )
    notes_ids = fields.One2many("note.note", "task_id", string="Notes",)
    ...
    def view_stock_moves(self):
        for rec in self:
            ...
            for move in rec.move_ids:
                ...
                stock_move_list += (
                    move.requisition_id.delivery_picking_id.move_lines.ids
                )
