...
class JobCostLine(models.Model):
    ...
    def _compute_actual_quantity(self):
        for rec in self:
            ...
            rec.actual_quantity = sum(
                [
                    p.order_id.state in ["purchase", "done"] and p.product_qty
                    for p in rec.purchase_order_line_ids
                ]
            )
    ...
    def _compute_actual_invoice_quantity(self):
        for rec in self:
            ...
            rec.actual_invoice_quantity = sum(
                [
                    p.invoice_id.state in ["open", "paid"] and p.quantity or 0.0
                    for p in rec.account_invoice_line_ids
                ]
            )

    direct_id = fields.Many2one("job.costing", string="Job Costing")
    product_id = fields.Many2one(
        "product.product", string="Product", copy=False, required=True,
    )
    ...
    total_cost = fields.Float(
        string="Cost Price Sub Total", compute="_compute_total_cost", store=True,
    )
    ...
    purchase_order_line_ids = fields.One2many(
        "purchase.order.line", "job_cost_line_id",
    )
    timesheet_line_ids = fields.One2many("account.analytic.line", "job_cost_line_id",)
    account_invoice_line_ids = fields.One2many(
        "account.invoice.line", "job_cost_line_id",
    )
    actual_quantity = fields.Float(
        string="Actual Purchased Quantity", compute="_compute_actual_quantity",
    )
    ...
    actual_hour = fields.Float(
        string="Actual Timesheet Hours", compute="_compute_actual_hour",
    )
