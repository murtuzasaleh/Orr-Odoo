...
class PurchaseRequisition(models.Model):
    ...
    task_user_id = fields.Many2one(
        "res.users", related="task_id.user_id", string="Task / Job Order User"
    )
    ...
    work_cost_package_total = fields.Float(
        compute="compute_equipment_machine", string="Work Cost Package", store=True,
    )
    subcontract_total = fields.Float(
        compute="compute_equipment_machine", string="Subcontract Cost", store=True,
    )
