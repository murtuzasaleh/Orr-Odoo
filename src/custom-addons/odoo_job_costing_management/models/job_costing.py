...
class JobCosting(models.Model):
    ...
    def unlink(self):
        for rec in self:
            if rec.state not in ("draft", "cancel"):
                raise Warning(
                    _(
                        "You can not delete Job Cost Sheet which is not draft or cancelled."
                    )
                )
    ...
    def _compute_material_total(self):
        for rec in self:
            rec.material_total = sum(
                [(p.product_qty * p.cost_price) for p in rec.job_cost_line_ids]
            )
    ...
    def _compute_labor_total(self):
        for rec in self:
            rec.labor_total = sum(
                [(p.hours * p.cost_price) for p in rec.job_labour_line_ids]
            )
    ...
    def _compute_overhead_total(self):
        for rec in self:
            rec.overhead_total = sum(
                [(p.product_qty * p.cost_price) for p in rec.job_overhead_line_ids]
            )
    ...
    def _compute_jobcost_total(self):
        for rec in self:
            rec.jobcost_total = (
                rec.material_total + rec.labor_total + rec.overhead_total
            )
    ...
    start_date = fields.Date(
        string="Create Date", readonly=True, default=fields.Date.today(),
    )
    complete_date = fields.Date(string="Closed Date", readonly=True,)
    material_total = fields.Float(
        string="Total Material Cost", compute="_compute_material_total", store=True,
    )
    labor_total = fields.Float(
        string="Total Labour Cost", compute="_compute_labor_total", store=True,
    )
    overhead_total = fields.Float(
        string="Total Overhead Cost", compute="_compute_overhead_total", store=True,
    )
    jobcost_total = fields.Float(
        string="Total Cost", compute="_compute_jobcost_total", store=True,
    )
    ...
    account_invoice_line_ids = fields.One2many("account.invoice.line", "job_cost_id",)
    ...
    def action_done(self):
        for rec in self:
            rec.write(
                {"state": "done", "complete_date": date.today(),}
            )
