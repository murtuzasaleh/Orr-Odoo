...
class PurchaseOrderWizard(models.TransientModel):
    ...
    supplier_ids = fields.Many2many("res.partner", string="Suppliers", required=True,)
    product_line_ids = fields.One2many(
        "product.lines", "product_line_id", "Product Lines"
    )
    ...
    def _prepare_purchase_order(self, vendor):
        self.ensure_one()
        stock_pick_obj = self.env["stock.picking"].browse(
            self._context.get("active_ids", [])
        )

        fpos = (
            self.env["account.fiscal.position"]
            .with_context(company_id=vendor.company_id.id)
            .get_fiscal_position(vendor.id)
        )
...
class ProductLines(models.TransientModel):
    ...
    product_line_id = fields.Many2one("purchase.order.wizard" "Purchase Order wizard")
