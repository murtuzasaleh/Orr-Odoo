    ...
    def _prepare_refund(
        ...
        for field in ["supplier_invoice_number", "date_due"]:
            if field == "supplier_invoice_number":
                if invoice[field]:
                    values[field] = "Refund-{0}-{1}".format(description, invoice[field])
                else:
                    values[field] = "Refund-{0}-{1}".format(description, self.number)
            else:
...
class AccountInvoiceLine(models.Model):
    ...
    purchase_order_id = fields.Many2one(
        "purchase.order", string="Purchase Order", copy=False
    )
