...
class CustomerPortal(CustomerPortal):
    ...
    def portal_my_documents(
        ...
        values.update({"directorys": directorys, "page_name": "directory_page"})
        return request.render(
            "document_directory_myaccount_portal.portal_my_document_directory", values
        )

    @http.route(
        ["/my/directory_documents/<int:directory>"],
        type="http",
        auth="public",
        website=True,
    )
    def portal_directory_page(self, directory=None, access_token=None, **kw):
        ...
        if directory_id == request.env.ref(
            "document_directory_myaccount_portal.menu_directory_other_document"
        ):
            attachment_ids = (
        ...
        return request.render(
            "document_directory_myaccount_portal.portal_my_directory_document", values
        )
    ...
    @http.route(
        ["/my/directory_doc/<int:attachment>"], type="http", auth="public", website=True
    )
    def portal_directory_attachment_page(
        self, attachment=None, access_token=None, **kw
    ):
        partner = request.env.user.partner_id
        attachment_id = request.env["ir.attachment"].sudo().browse(attachment)
        if (
            partner.commercial_partner_id
            not in attachment_id.partner_ids.commercial_partner_id
        ):
        ...
        return request.render(
            "document_directory_myaccount_portal.portal_attachment_page", values
        )
    ...
    @http.route(
        [
            ...
            "/my/document/<string:model>/<int:id>/<string:field>/<string:filename>",
        ],
        type="http",
        auth="public",
    )
