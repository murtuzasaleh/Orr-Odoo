        ...
        for move in self:
            ...
            if move.journal_id and move.journal_id.support_creditcard_transactions:
                if not move.journal_id.partner_id.property_account_payable_id:
                    raise Warning(
                        _("Payable Account not configured for %s .")
                ...
                if proceed:
                    ...
                    for move_line in move.line_ids:
                        ...
                        if (
                            ...
                            == move_line.partner_id.property_account_payable_id.id
                            or move_line.account_id.id
                            != move.journal_id.default_debit_account_id.id
                        ):
                            account = (
                                move.journal_id.partner_id.property_account_payable_id
                                and move.journal_id.partner_id.property_account_payable_id.id
                            )
                        else:
