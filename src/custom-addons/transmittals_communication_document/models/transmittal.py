...
class TransmittalDocument(models.Model):
    ...
    sending_date = fields.Date(string="Date of Sending", required=True,)
    sender_company_id = fields.Many2one(
        "res.partner", string="Sender Company", required=True,
    )
    receiver_company_id = fields.Many2one(
        "res.partner", string="Receiver Company", copy=False
    )
    project_id = fields.Many2one("project.project", string="Project", required=True,)
    job_costsheet_id = fields.Many2one("job.costing", string="Job Costsheet",)
    analytic_account_id = fields.Many2one(
        "account.analytic.account", string="Analytic Account", required=True,
    )
    reason_for_send = fields.Text(string="Reason for Sending",)
    deadline = fields.Date(string="Deadline")
    description = fields.Text(string="Description Taken by Recipient")
    user_id = fields.Many2one(
        "res.users", default=lambda self: self.env.user, string="Responsible User",
    )
    ...
    transmittal_line_ids = fields.One2many(
        "transmittal.document.line", "transmittal_id", string="Documents", copy=True
    )
    job_order_id = fields.Many2one("project.task", string="Job Order")
    document_type = fields.Selection(
        [("submittal_type", "Submittal"), ("transmittal_type", "Transmittal")],
    ...
    def action_send_transmittal_document(self):
        ...
        try:
            template_id = ir_model_data.get_object_reference(
                ...
                "email_template_transmittal_submittal_doc",
            )[1]
        except ValueError:
            template_id = False
        try:
...
class TransmittalDocumentLine(models.Model):
    ...
    transmit_type_id = fields.Many2one(
        "transmittal.type", string="Type", required=True,
    )
    transmit_medium_id = fields.Many2one(
        "transmittal.medium", string="Channel", required=True,
    )
    name = fields.Char(string="Name", required=True)
    description = fields.Char(string="Description", required=True,)
    transmittal_id = fields.Many2one(
        "transmittal.document", string="Transmittal Document"
    )
