...
class Project(models.Model):
    ...
    transmittion_ids = fields.One2many(
        "transmittal.document", "project_id", string="Transmittal Documents",
    )
    transmittal_document_count = fields.Integer(
        compute="_compute_transmittal_documents", string="Transmittal Document Count"
    )
    ...
    def action_transmittal_documents(self):
        self.ensure_one()
        action = self.env.ref(
            "transmittals_communication_document.action_transmittal_document"
        ).read()[0]
...
class ProjectTask(models.Model):
    ...
    transmittion_ids = fields.One2many(
        "transmittal.document", "job_order_id", string="Transmittal Documents",
    )
    transmittal_document_count = fields.Integer(
        compute="_compute_transmittal_documents", string="Transmittal Document Count"
    )
