...
class MachineRepairSupport(models.Model):
    ...
    def create(self, vals):
        ...
        if vals.get("name", "New") == "New":
            vals["name"] = (
                self.env["ir.sequence"].next_by_code("machine.repair.support") or "New"
            )
    ...
    timesheet_line_ids = fields.One2many(
        "account.analytic.line", "repair_request_id", string="Timesheets",
    )
    ...
    team_id = fields.Many2one(
        ...
        default=lambda self: self.env["machine.support.team"]
        .sudo()
    ...
    product_id = fields.Many2one(
        "product.product", domain="[('is_machine', '=', True)]", string="Product"
    )
    brand = fields.Char(string="Brand")
    color = fields.Char(string="Color")
    model = fields.Char(string="Model")
    year = fields.Char(string="Year")
    accompanying_items = fields.Text(string="Accompanying Items",)
    damage = fields.Text(string="Damage",)
    warranty = fields.Boolean(string="Warranty",)
    img1 = fields.Binary(string="Images1",)
    img2 = fields.Binary(string="Images2",)
    img3 = fields.Binary(string="Images3",)
    img4 = fields.Binary(string="Images4",)
    img5 = fields.Binary(string="Images5",)
    ...
    cosume_part_ids = fields.One2many(
        "product.consume.part", "machine_id", string="Produ Ctconsume Part"
    )
    nature_of_service_id = fields.Many2one("service.nature", string="Nature Of service")
    lot_id = fields.Many2one("stock.production.lot", string="Lot")
    website_brand = fields.Char(string="Website Brand")
    website_model = fields.Char(string="Website Model")
    website_year = fields.Char(string="Website Year")
    #     @api.multi
    ...
    def show_work_order_task(self):
        for rec in self:
            ...
            res = res.read()[0]
            res["domain"] = str(
                [("task_type", "=", "work_order"), ("machine_ticket_id", "=", rec.id)]
            )
            res["context"] = {
                "default_machine_ticket_id": rec.id,
                "default_task_type": "work_order",
            }
        return res
...
class HrTimesheetSheet(models.Model):
    ...
    #     )
    billable = fields.Boolean(string="Chargable?", default=True,)
