...
class ProjectTask(models.Model):
    ...
    task_type = fields.Selection(
        selection=[("diagnosys", "Diagnosys"), ("work_order", "Work Order"),],
        string="Type",
        readonly=True,
    )
    repair_estimation_line_ids = fields.One2many(
        "repair.estimation.lines", "task_id", string="Repair Estimation Lines"
    )
            ...
            for line in rec.repair_estimation_line_ids:
                ...
                if order_id.pricelist_id:
                    price_unit, rule_id = order_id.pricelist_id.get_product_price_rule(
                        line.product_id, line.qty or 1.0, order_id.partner_id
                    )
