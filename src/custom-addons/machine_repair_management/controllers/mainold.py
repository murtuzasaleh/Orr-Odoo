...
class MachineRepairSupport(http.Controller):
    @http.route(
        ["/page/machine_repair_support_ticket"],
        type="http",
        auth="public",
        website=True,
    )
    def open_machine_repair_request(self, **post):
        ...
        return request.render(
            "machine_repair_management.website_machine_repair_support_ticket",
            {"service_ids": service_ids, "srvice_type_ids": srvice_type_ids,},
        )

    @http.route(
        ["/machine_repair_management/request_submitted"],
        type="http",
        auth="public",
        methods=["POST"],
        website=True,
    ...
    def request_submitted(self, **post):
        ...
        if Partner:
            ...
            values = {"support": support, "user": request.env.user}
            ...
            return request.render(
                "machine_repair_management.support_invalid", {"user": request.env.user}
            )

    @http.route(
        ["/machine_repair_management/invite"],
        auth="public",
        website=True,
        methods=["POST"],
    )
    def index_user_invite(self, **kw):
        ...
        if user_exist:
            return http.request.render(
                "machine_repair_management.user_alredy_exist", vals
            )
        value = {
            "name": name,
            "email": email,
        ...
        base_url = (
            ...
            + "/page/machine_repair_management.user_thanks"
        )
        ...
        reject_url = (
            ...
            + "/page/machine_repair_management.user_thanks_reject"
        )
        ...
        issue_template = http.request.env.ref(
            "machine_repair_management.email_template_machine_ticket"
        )
    ...
    @http.route(
        ["/machine_repair_email/feedback/<int:order_id>"],
        type="http",
        auth="public",
        website=True,
    )
    def feedback_email(self, order_id, **kw):
        ...
        return request.render(
            "machine_repair_management.machine_repair_feedback", values
        )

    @http.route(
        ["/machine_repari/feedback/"], methods=["POST"], auth="public", website=True
    )
...
class website_account(website_account):
    def _prepare_portal_layout_values(self):  # odoo11
        ...
        values.update(
            {
                "repair_request_count": request.env[
    ...
    @http.route(
        ["/my/repair_requests", "/my/repair_requests/page/<int:page>"],
        type="http",
        auth="user",
        website=True,
    )
    def portal_my_repair_request(self, page=1, **kw):
        ...
        repair_request = support_obj.sudo().search(
        ...
        return request.render(
            "machine_repair_management.display_repair_requests", values
        )

    @http.route(
        ['/my/repair_request/<model("machine.repair.support"):repair_request>'],
        type="http",
        auth="user",
        website=True,
    )
    def my_repair_request(self, repair_request=None, access_token=None, **kw):
        attachment_list = request.httprequest.files.getlist("attachment")
        support_obj = (
            http.request.env["machine.repair.support"].sudo().browse(repair_request.id)
        )
        for image in attachment_list:
        ...
        if len(attachment_list) > 0:
            ...
            return http.request.render(
                "machine_repair_management.successful_ticket_send", {}
            )
        ...
        return request.render(
            "machine_repair_management.display_repair_request_from",
            {
