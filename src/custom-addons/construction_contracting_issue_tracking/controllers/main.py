...
class ConstructionIssue(http.Controller):
    @http.route(
        ["/construction_contracting/ticket_submitted"],
        type="http",
        auth="public",
        methods=["POST"],
        website=True,
    ...
    def ticket_submitted(self, **post):
        ...
        Partner = (
            ...
            .sudo()
            .search([("email", "=", post["email"])], limit=1)
        )
        ...
        if Partner:
            ...
            if len(attachment_list) > 0:
                group_msg = (
                    "Customer has sent %s attachments to this Issue. Name of attachments are: "
                    % (len(attachment_list))
                )
                ...
                construction_id.sudo().message_post(
                    body=_(group_msg), message_type="comment"
                )

            return request.render(
                "construction_contracting_issue_tracking.thanks_mail_send", values
            )
        else:
            return request.render(
                "construction_contracting_issue_tracking.construction_mail_invalid", {}
            )

    @http.route(
        ["/construction_contracting_issue_tracking/invite"],
        auth="public",
        website=True,
        methods=["POST"],
    )
    def index_user_invite(self, **kw):
        ...
        user_exist = (
            request.env["res.users"].sudo().search([("login", "=", str(email))])
        )
        vals = {
            "user_id": user_exist,
        }
        if user_exist:
            return http.request.render(
                "construction_contracting_issue_tracking.user_alredy_exist", vals
            )
        value = {
            "name": name,
            "email": email,
        ...
        base_url = (
            http.request.env["ir.config_parameter"].get_param(
                "web.base.url", default="http://localhost:8069"
            )
            + "/page/construction_contracting_issue_tracking.user_thanks"
        )
        ...
        reject_url = (
            ...
            + "/page/construction_contracting_issue_tracking.user_thanks_reject"
        )
        rejected_url = "%s?user_info=%s" % (reject_url, user_info_id.id)
        local_context = http.request.env.context.copy()
        issue_template = http.request.env.ref(
            "construction_contracting_issue_tracking.email_template_helpdesk_ticket"
        )
        ...
        issue_template.sudo().with_context(local_context).send_mail(
            request.uid, force_send=True
        )

    @http.route(
        ["/construction_email/feedback/<int:ticket_id>"],
        type="http",
        auth="public",
        website=True,
    )
    def feedback_email(self, ticket_id, **kw):
        ...
        return request.render(
            "construction_contracting_issue_tracking.construction_feedback", values
        )

    @http.route(["/construction/feedback"], type="http", auth="public", website=True)
    def start_rating(self, **kw):
        ...
        customer_msg = _(
            ticket_obj.partner_id.name
            + "has send this feedback rating is %s and comment is %s"
        ) % (kw["star"], kw["comment"],)
        ticket_obj.sudo().message_post(body=customer_msg)
        return http.request.render(
            "construction_contracting_issue_tracking.successful_construction_feedback"
        )
...
class CustomerPortal(CustomerPortal):
    ...
    @http.route(
        ["/my/construction_tickets", "/my/construction_tickets/page/<int:page>"],
        type="http",
        auth="user",
        website=True,
    )
    def portal_my_ticket(
        self,
        page=1,
        date_begin=None,
        date_end=None,
        ...
        **kw
        ...
        searchbar_inputs = {
            "title": {
                "input": "title",
                "label": _('Search <span class="nolabel"> (in Title)</span>'),
            },
        ...
        tickets = construction_obj.sudo().search(
        ...
        return request.render(
            "construction_contracting_issue_tracking.display_construction_tickets",
            values,
        )

    @http.route(
        ['/my/construction_ticket/<model("construction.ticket"):ticket>'],
        type="http",
        auth="user",
        website=True,
    )
    def my_ticket(self, ticket=None, **kw):
        attachment_list = request.httprequest.files.getlist("attachment")
        construction_obj = (
            http.request.env["construction.ticket"].sudo().browse(ticket.id)
        )
        for image in attachment_list:
        ...
        if len(attachment_list) > 0:
            group_msg = (
                "Customer has sent %s attachments to this Construction ticket. Name of attachments are: "
                % (len(attachment_list))
            )
            for attach in attachment_list:
                group_msg = group_msg + "\n" + attach.filename
            ...
            construction_obj.sudo().message_post(
                body=_(group_msg),
            ...
            return http.request.render(
                "construction_contracting_issue_tracking.successful_construction_ticket_send",
                {},
            )
        ...
        return request.render(
            "construction_contracting_issue_tracking.display_construction_ticket",
            {"ticket": ticket, "user": request.env.user},
        )
