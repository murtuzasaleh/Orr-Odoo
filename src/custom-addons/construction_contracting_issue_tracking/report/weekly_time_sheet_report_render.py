...
class WeeklyTimeSheetReport(models.AbstractModel):
    ...
    def render_html(self, docids, data=None):
        ...
        start_date = datetime.date(
            datetime.strptime(weekly_timesheet_wizard_ids.start_date, "%Y-%m-%d")
        )
        end_date = datetime.date(
            datetime.strptime(weekly_timesheet_wizard_ids.end_date, "%Y-%m-%d")
        )
        ...
        return self.env["report"].render(
            "construction_contracting_issue_tracking.report_weekly", docargs
        )
