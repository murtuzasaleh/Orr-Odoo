...
class HelpdeskSupport(models.Model):
    ...
    def create(self, vals):
        ...
        if vals.get("name", "New") == "New":
            vals["name"] = (
                self.env["ir.sequence"].next_by_code("helpdesk.support") or "New"
            )
        ...
        return super(
            HelpdeskSupport, self.with_context(context, mail_create_nolog=True)
        ).create(vals)
    ...
    def set_to_close(self):
        stage_id = self.env["helpdesk.stage.config"].search(
            [("stage_type", "=", "closed")]
        )
        if self.is_close != True:
            ...
            template = self.env.ref(
                "website_helpdesk_support_ticket.email_template_helpdesk_ticket"
            )
    ...
    def _default_stage_id(self):
        team = (
            self.env["support.team"].sudo()._get_default_team_id(user_id=self.env.uid)
        )
    ...
    def _stage_find(self, team_id=False, domain=None, order="sequence"):
        ...
        return self.env["helpdesk.stage.config"].search(
    ...
    type_ticket_id = fields.Many2one("ticket.type", string="Type of Issue", copy=False,)
    ...
    timesheet_line_ids = fields.One2many(
        "account.analytic.line", "support_request_id", string="Timesheets",
    )
    ...
    team_id = fields.Many2one(
        ...
        default=lambda self: self.env["support.team"]
        .sudo()
    ...
    invoice_line_ids = fields.One2many(
        "support.invoice.line", "support_id", string="Invoice Lines",
    )
    journal_id = fields.Many2one("account.journal", string="Invoice Journal",)
    invoice_id = fields.Many2one(
        "account.invoice", string="Invoice Reference", copy="False",
    )
    ...
    subject_type_id = fields.Many2one(
        "type.of.subject", string="Type of Subject", copy=True,
    )
