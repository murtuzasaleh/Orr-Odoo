...
class AccountAnalyticAccount(models.Model):
    ...
    @api.multi
    @api.depends("total_purchase_hours", "total_consumed_hours")
    ...
    timesheet_custom_ids = fields.One2many(
        "account.analytic.line", "account_id", string="Timesheet Custom"
    )
...
class PreparedPurchaseHhours(models.Model):
    ...
    prepared_hours_id = fields.Many2one("account.analytic.account", string="Contract")
    sale_order_id = fields.Many2one("sale.order", "Sales Order", required=True)
    date = fields.Date("Date of Sales Order", required=True)
    purchased_hours = fields.Float("Purchased Hours", required=True)
