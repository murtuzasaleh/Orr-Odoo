...
class ConstructionTicket(models.Model):
    ...
    def _write(
        self, vals
    ):  # this is to fix access error on stage write with other records.
    ...
    def create(self, vals):
        ...
        if vals.get("name", "New") == "New":
            vals["name"] = (
                self.env["ir.sequence"].next_by_code("construction.ticket") or "New"
            )
        ...
        if not vals.get("partner_id", False) and vals.get("email", ""):
            partner = (
                ...
                .sudo()
                .search([("email", "=", vals["email"])], limit=1)
            )
            if partner:
        ...
        if vals.get("team_id") and not vals.get("team_leader_id"):
            vals["team_leader_id"] = (
                self.env["construction.team"].browse(vals.get("team_id")).leader_id.id
            )

        # context: no_log, because subtype already handle this
        return super(
            ConstructionTicket, self.with_context(context, mail_create_nolog=True)
        ).create(vals)
    ...
    def set_to_close(self):
        stage_id = self.env["construction.stage.config"].search(
            [("stage_type", "=", "closed")]
        )
        if self.is_close != True:
            ...
            template = self.env.ref(
                "construction_contracting_issue_tracking.email_template_construction_ticket"
            )
    ...
    def _stage_find(self, team_id=False, domain=None, order="sequence"):
        ...
        return self.env["construction.stage.config"].search(
            search_domain, order=order, limit=1
        )

    name = fields.Char(
        string="Number", required=False, default="New", copy=False, readonly=True,
    )
    ...
    #    )
    type_ticket_id = fields.Many2one(
        "construction.ticket.type", string="Type of Issue", copy=False,
    )
    description = fields.Text(string="Description")
    priority = fields.Selection(
        [("0", "Low"), ("1", "Middle"), ("2", "High")], string="Priority",
    )
    partner_id = fields.Many2one("res.partner", string="Customer",)
    request_date = fields.Datetime(
        string="Create Date", default=fields.Datetime.now, copy=False,
    )
    ...
    total_spend_hours = fields.Float(
        string="Total Hours Spent", compute="_compute_total_spend_hours", store=True,
    )
    ...
    team_id = fields.Many2one(
        ...
        default=lambda self: self.env["construction.team"]
        .sudo()
        ._get_default_team_id(user_id=self.env.uid),
    )
    ...
    invoice_line_ids = fields.One2many(
        "construction.invoice.line", "construction_id", string="Invoice Lines",
    )
    journal_id = fields.Many2one("account.journal", string="Invoice Journal",)
    invoice_id = fields.Many2one(
        "account.invoice", string="Invoice Reference", copy="False", readonly=True,
    )
    ...
    comment = fields.Text(string="Customer Comment", readonly=True,)
    ...
    subject_type_id = fields.Many2one(
        "construction.type.of.subject", string="Type of Issue Subject", copy=True,
    )
    ...
    stage_type = fields.Selection("Type", store=True, related="stage_id.stage_type",)
    ...
    total_purchase_hours = fields.Float(
        string="Total Purchase Hours", compute="compute_total_hours", store=True,
    )
    total_consumed_hours = fields.Float(
        string="Total Consumed Hours", compute="compute_total_hours", store=True,
    )
    remaining_hours = fields.Float(
        string="Remaining Hours", compute="compute_total_hours", store=True,
    )

    # JOBCOSTING CONTRIBUTION
    job_order_id = fields.Many2one("project.task", string="Job Order",)
    jobcost_sheet_id = fields.Many2one("job.costing", string="Job Cost Sheet",)
    ...
    def unlink(self):
        for rec in self:
            if rec.stage_id.stage_type != "new":
                raise Warning(
                    _("You can not delete record which are not in draft state.")
                )
            ...
            for line in rec.invoice_line_ids:
                if line.is_invoice:
                    ...
                    if not account:
                        raise UserError(
                            _(
                                'Please define expense account for this product: "%s" (id:%d) - or for its category: "%s".'
                            )
    ...
    def _prepare_invoice(self):
        ...
        if not self.journal_id:
            raise UserError(
                _("Please configure an accounting sale journal for this company.")
            )
        invoice_vals = {
            "name": self.name or "",
            "origin": self.name,
    ...
    def message_new(self, msg_dict, custom_values=None):
        ...
        return super(ConstructionTicket, self).message_new(
