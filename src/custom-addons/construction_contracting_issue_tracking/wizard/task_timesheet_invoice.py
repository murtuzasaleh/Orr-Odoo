...
class TimsheetTaskInvoice(models.TransientModel):
    ...
    def _prepare_invoice_line(self, account_lines, invoice):
        ...
        for account_line in account_lines:
            ...
            time_in = (
                str(int(account_line.time_in))
                + ":"
                + str(int(fmod(account_line.time_in, 1) * 60))
            )
            time_out = (
                str(int(account_line.time_out))
                + ":"
                + str(int(fmod(account_line.time_out, 1) * 60))
            )
            name = ""
            ...
            if account_line.task_id and account_line.task_id.product_id_construction:
                ...
                account_line.project_id
                and account_line.project_id.product_id_construction
            ):
                product_id_helpdesk = account_line.project_id.product_id_construction.id
            else:
                product_id_helpdesk = (
                    account_line.project_id.partner_id.product_id_construction.id
                )
            ...
            if account_line.task_id:
                ...
                account_analytic_id = (
                    account_line.task_id.project_id.analytic_account_id.id
                )
            else:
            ...
            account = self.env["ir.property"].get(
                "property_account_income_categ_id", "product.category"
            )
            if not account:
                raise UserError(
                    _(
                        "Please configure Default Income account for Product income: `property_account_income_categ_id`."
                    )
                )
            res = {
                "name": _(name),
    ...
    def create_timesheet_invoice(self):
        ...
        for res_line in analytic_account_line:
            if res_line.invoiced_created:
                raise Warning(
                    _(
                        "Invoice can not be created since some of Timesheet lines are already invoiced so please check."
                    )
                )
            ...
            if (res_line.partner_id.id, res_line.project_id.id) not in customer_invoice:
                customer_invoice[(res_line.partner_id.id, res_line.project_id.id)] = [
                    res_line
                ]
            else:
                customer_invoice[
                    (res_line.partner_id.id, res_line.project_id.id)
                ].append(res_line)
        invoice_ids = []
        for line in customer_invoice:
            ...
            invoice = self._prepare_invoice(
                customer_invoice[line], partner_id, project_id
            )
            if invoice is None:
                raise Warning(_("No invoices created."))
            invoice_ids.append(invoice.id)
            self._prepare_invoice_line(customer_invoice[line], invoice)
            for analyitic_line in customer_invoice[
                line
            ]:  # set boolean invoiced to true so next time it will be skip
