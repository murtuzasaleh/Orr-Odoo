...
AVAILABLE_PRIORITIES = [("0", "Very Low"), ("1", "Low"), ("2", "Normal"), ("3", "High")]


class MaintenanceRequestWizard(models.TransientModel):
    _name = "maintenance.request.wizard"

    maintenance_name = fields.Char(string="Name", required=True,)
    maintenance_equipment_id = fields.Many2one(
        "maintenance.equipment", string="Maintenance Equipment", required=True,
    )
    mainteance_team_id = fields.Many2one(
        "maintenance.team", string="Maintenance Team", required=True,
    )
    maintenance_date = fields.Datetime("Schedule Date",)
    maintenance_duration = fields.Float("Duration (Hours)",)
    mainteance_type = fields.Selection(
        [("corrective", "Corrective"), ("prevantive", "Prevantive")], string="Type",
    )
    rating = fields.Selection(AVAILABLE_PRIORITIES, select=True, string="Priority",)
