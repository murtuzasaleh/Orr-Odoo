...
class JobInstruction(models.Model):
    ...
    instruction_type = fields.Many2one(
        "instruction.type", string="Job Instruction", required=True,
    )
    quality_checklist_ids = fields.Many2many(
        "quality.checklist", string="Quality Checklist", readonly=False,
    )
    job_id = fields.Many2one("project.task", string="Job",)
    image_1 = fields.Binary(
        string="Instruction Photo1", related="instruction_type.image_1",
    )
    image_2 = fields.Binary(
        string="Instruction Photo2", related="instruction_type.image_2",
    )
    image_3 = fields.Binary(
        string="Instruction Photo3", related="instruction_type.image_3",
    )
    image_4 = fields.Binary(
        string="Instruction Photo4", related="instruction_type.image_4",
    )
    image_5 = fields.Binary(
        string="Instruction Photo5", related="instruction_type.image_5",
    )
    ...
    instruction_date = fields.Date(string="Date", default=fields.date.today(),)
    ...
    def print_job_instruction(self):
        return self.env.ref(
            "job_order_card_instruction.job_instruction_information"
        ).report_action(self)
