...
class SaleEstimateJob(models.Model):
    ...
    estimate_date = fields.Date(
        "Date", required=True, copy=False, default=fields.date.today(),
    )
    ...
    payment_term_id = fields.Many2one(
        "account.payment.term", string="Payment Terms", oldname="payment_term"
    )
    ...
    user_id = fields.Many2one(
        "res.users", "Sales Person", default=lambda self: self.env.user,
    )
    team_id = fields.Many2one("crm.team", "Sales Team",)
    quotation_id = fields.Many2one(
        "sale.order", "Sales Quotation", readonly=True, copy=False,
    )
    ...
    estimate_total = fields.Float(
        string="Total Job Estimate", compute="_compute_job_estimate_total", store=True,
    )
    job_type_ids = fields.Many2many("estimate.job.type", string="Job Types",)
