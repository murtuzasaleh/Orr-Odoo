...
class SaleEstimatelineJob(models.Model):
    ...
    estimate_id = fields.Many2one("sale.estimate.job", string="Sale Estimate",)
    ...
    analytic_id = fields.Many2one(
        "account.analytic.account", "Analytic Account", store=True,
    )
    ...
    def _compute_tax_id(self):
        for line in self:
            ...
            line.tax_id = (
                fpos.map_tax(taxes, line.product_id, line.estimate_id.partner_id)
                if fpos
                else taxes
            )
    ...
    def _get_display_price(self, product):
        if self.estimate_id.pricelist_id.discount_policy == "without_discount":
            ...
            return from_currency.compute(
                product.lst_price, self.estimate_id.pricelist_id.currency_id
            )
