...
class Project(models.Model):
    ...
    project_team_id = fields.Many2one(
        "project.team.custom", string="Project Team", copy=False
    )
    project_engineer_id = fields.Many2one(
        "res.users", string="Site Engineer / Supervisor", copy=False
    )
    store_officer_ids = fields.Many2many(
        "res.users", "team_store_officer_res_user", string="Store Officers", copy=False
    )
    ...
    team_member_ids = fields.Many2many(
        "res.users", "team_project_team_res_user", string="Team Members", copy=False
    )
...
class ProjectTask(models.Model):
    ...
    def _onchange_project(self):
        if self.project_id:
            ...
            self.project_engineer_id = (
                self.project_id.project_team_id.project_engineer_id.id
            )
            self.store_officer_ids = (
                self.project_id.project_team_id.store_officer_ids.ids
            )
            self.security_guard_ids = (
                self.project_id.project_team_id.security_guard_ids.ids
            )
    ...
    store_officer_ids = fields.Many2many(
        "res.users", "task_store_officer_res_user", string="Store Officers", copy=False
    )
    security_guard_ids = fields.Many2many(
        "res.users",
    ...
    team_member_ids = fields.Many2many(
        "res.users", "task_project_team_res_user", string="Team Members", copy=False
    )
